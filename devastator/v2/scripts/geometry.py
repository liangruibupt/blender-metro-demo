...
class Workshop:
    ...
    def track(self, name, pos, length, radius, width, parent, rot=(0, 0, 0)):
        ...
        reverse = self.group("Reverse crawler guard", (0, width*.60, .04), group,
                             (0, 0, math.pi))
        self.panel("Reverse guard plate", (0, 0, 0), half*1.55, radius*.88, reverse)
        self.hazard((0, -.11, 0), half*1.30, radius*.60, reverse)
        return group
