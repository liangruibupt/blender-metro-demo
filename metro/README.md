...
This task lives in `metro/`. Paths in the deliverables list are relative to this
directory; run the commands below from the repository root. The separate
[Optimus study](../optimus/README.md) lives in `optimus/`.
...
- `assets/station.glb`: original station environment, composed with the
...
- `assets/metro.glb`: portable model with 12 independently sliding door
...
Open `http://127.0.0.1:5187/metro/`. Use another port if 5187 is occupied.
...
```sh
/Users/ruiliang/Documents/workspaces/venv/bin/python metro/scripts/check_assets.py
```
...
/Applications/Blender.app/Contents/MacOS/Blender \
  ...
  --python metro/scripts/build_metro.py -- --render
```
...
/Applications/Blender.app/Contents/MacOS/Blender \
  --background --factory-startup --python-exit-code 1 \
  --python metro/scripts/build_station.py -- --render
```
...
uses `codex/metro-demo` as its default branch. `PR_DRAFT.md` is a historical
draft retained with the Metro task; it is not the current branch/deployment
configuration. Further pushes must pass the configured repository security
checks. See the [repository README](../README.md) for the shared build.
...
No remote deployment is performed by the local build.
